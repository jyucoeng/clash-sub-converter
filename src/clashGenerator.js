...
export class ClashGenerator {
    ...
    generate(proxies, useMeta = false) {
        ...
        const proxyNames = uniqueProxies.map(p => p.name);

        // 去重节点名称列表
        const uniqueProxyNames = [...new Set(proxyNames)];
        const proxyGroups = this.generateProxyGroups(uniqueProxyNames);
...
`;
        ...
        // 添加规则
        yaml += '\nrules:\n';
        const rules = useMeta ? this.generateRulesWithProviders() : this.generateRules();
        for (const rule of rules) {
            yaml += `  - ${rule}\n`;
        }

        // 如果是 meta 格式，添加 rule-providers
        if (useMeta) {
            yaml += '\nrule-providers:\n';
